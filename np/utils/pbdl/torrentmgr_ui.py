...
def create_torrentmgr_ui():
	...
	media_info_actions = [sg.Combo(['-rotten tomatoes-', '-TMDB-', '-IMDB-'], '-TMDB-' , enable_events=True, key='-LOOKUP_TYPE-'), sg.Button('Start Torrent!', key='-START_TORRENT-'), sg.Button('Lookup!', key='-LOOKUP-'), sg.Button('Migrate Files', key='-Migrate Files-'), sg.Button('Remove'), sg.Button('Remove+Delete'), sg.Button('Exclude')]
