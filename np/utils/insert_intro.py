...
def fix3d(filepath):
	com = (f"ffprobe -v error -select_streams v:0 -show_entries stream=width,height \"{filepath}\"")
	...
	newname = f"{fname}.fixed{ext}"
	com = (f"ffmpeg -i \"{filepath}\" -strict unofficial -metadata:s:v stereo_mode=left_right -profile:v high -level:v 4.0 -vf scale={nw}:{nh} -r 30 \"{newname}\"&")
	ret = subprocess.call(com, shell=True).decode().strip()
	return ret
