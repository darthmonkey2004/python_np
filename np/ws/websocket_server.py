...
from np.ws.thread import WebsocketServerThread
...
class WebsocketServer(ThreadingMixIn, TCPServer, API):
	...
	def __init__(self, port=0, loglevel=logging.WARNING, key=None, cert=None):
		self.conf = readConf()
		self.state = self.conf['remote']['server']['state']
		if self.state == 0:
			self.host = '127.0.0.1'
		elif self.state == 1:
			self.host = '0.0.0.0.'
		self.port = self.conf['remote']['server']['port']
		logger.setLevel(loglevel)
		TCPServer.__init__(self, (self.host, self.port), WebSocketHandler)
